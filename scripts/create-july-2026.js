...
const JULY_2026 = require("./july-2026-data");
...
(async () => {
  if (!process.env.DATABASE_URL) {
    console.error("DATABASE_URL not set."); await sql.end(); process.exit(1);
  }

  // 1. Abort if booklet already exists
  const existing = await sql`SELECT id FROM booklets WHERE month = 7 AND year = 2026 LIMIT 1`;
  if (existing.length) {
    console.error(`July 2026 booklet already exists (ID ${existing[0].id}). Aborting.`);
    await sql.end(); process.exit(1);
  }

  // 2. Randomize from existing image pool (same scheme as other months)
  ...
  const [booklet] = await sql`
    ...
    VALUES (
      'My Life & My Cashflow Affirmations - July 2026',
      7, 2026,
      'Daily affirmations for July 2026',
      '#E3A24B'
    )
    RETURNING *
  `;
  ...
  // 4. Insert 31 affirmations (titles now, bodies to follow)
  console.log("\nInserting affirmations...");
  for (let i = 0; i < JULY_2026.length; i++) {
    const { day, title, content } = JULY_2026[i];
  ...
  console.log(`\nDone. July 2026 booklet (ID ${booklet.id}) created with ${JULY_2026.length} affirmations.`);
  await sql.end();
